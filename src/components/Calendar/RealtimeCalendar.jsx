...
const RealtimeCalendar = () => {
  ...
  const STORAGE_KEY = 'bookings_realtime_sync';
  
  // Load bookings from both sources and merge
  const loadBookings = async () => {
    try {
      console.log('🔄 Loading bookings from all sources...');
      ...
      if (error) {
        ...
        calculateStats(localBookings);
        return;
      }

      console.log('☁️ Supabase bookings:', supabaseBookings?.length || 0);

      // Convert Supabase format
      const convertedSupabase = (supabaseBookings || []).map(booking => ({
        id: booking.id,
        name: booking.name,
      ...
      // Smart merge - prefer Supabase if it has more data
      const merged = mergeBookingsIntelligently(localBookings, convertedSupabase);
      ...
      console.log('✅ Final merged bookings:', merged.length);
  ...
  // Smart merge that prevents data loss
  const mergeBookingsIntelligently = (local, remote) => {
    const allBookings = new Map();
    
    // Add all local bookings first
    local.forEach(booking => {
      allBookings.set(booking.id, { ...booking, source: 'local' });
    });
    
    // Add remote bookings, but don't overwrite newer local ones
    remote.forEach(booking => {
      const existing = allBookings.get(booking.id);
      if (!existing || new Date(booking.createdAt) >= new Date(existing.createdAt)) {
        allBookings.set(booking.id, { ...booking, source: 'remote' });
      }
    });
  ...
  // Save to both local and Supabase immediately
  const saveBooking = async (bookingData) => {
    try {
      console.log('💾 Saving booking:', bookingData.name);
      
      // 1. Save to local storage IMMEDIATELY
      const currentBookings = [...bookings, bookingData];
      setBookings(currentBookings);
      localStorage.setItem(STORAGE_KEY, JSON.stringify(currentBookings));
      calculateStats(currentBookings);
      
      console.log('✅ Saved locally immediately');
      ...
      if (error) {
        ...
        toast.error('Gemt lokalt - sync fejlede');
      } else {
        console.log('☁️ Saved to Supabase successfully');
  ...
  const deleteBooking = async (id) => {
    try {
      ...
      console.log('✅ Deleted locally immediately');
      ...
      if (error) {
        ...
        toast.error('Slettet lokalt - sync fejlede');
      } else {
        console.log('☁️ Deleted from Supabase successfully');
  ...
  useEffect(() => {
    console.log('🚀 Setting up real-time calendar...');
    ...
    const handleStorageChange = (e) => {
      if (e.key === STORAGE_KEY) {
        console.log('📱 Cross-tab change detected');
        loadBookings();
      }
    };
  ...
  const createBooking = async (acceptDoubleBooking = false) => {
    setLoading(true);
    try {
      const booking = {
