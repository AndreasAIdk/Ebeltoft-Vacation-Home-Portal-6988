...
const { FiCalendar, FiPlus, FiChevronLeft, FiChevronRight, FiUsers, FiClock, FiMessageCircle, FiAlertTriangle, FiTrash2, FiDownload } = FiIcons;

const SimpleVisualCalendar = () => {
  const { user } = useAuth();
  ...
  // Load bookings - simple and working approach
  const loadBookings = async () => {
    try {
      // Always start with localStorage (never lose data)
      ...
      if (!error && supabaseBookings) {
        ...
        // Update if Supabase has data
        if (converted.length >= localBookings.length) {
  ...
  // Save booking - the working version
  const saveBooking = async (bookingData, acceptDoubleBooking = false) => {
    try {
      ...
      const updatedBookings = [...bookings, finalBooking];
        ...
        {showForm && (
          ...
          >
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Kommentar (valgfrit)</label>
                <textarea
...
export default SimpleVisualCalendar;